...
configfile = open("./data/config.json").read()
config = json.loads(configfile)

selenium_logs_path = config["Logs"]["webdriver-log"]
...
class AioHcaptcha:
    ...
    async def _solve_hsw(self, sc: str) -> str:
        ...
        def _hsw():
            ...
            driver = Chrome(service=Service(**self.chromedriver_args, service_args=["--verbose", f"--log-path={selenium_logs_path}"]), options=options) #WEBDRIVERLOG
